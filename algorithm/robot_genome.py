...
class RobotGenome:
    def __init__(self, sensor_num: int = 12, motor_num: int = 2, hidden_layer_size: int = 4) -> None:
        ...
        self._input_layer_weights = np.array([[random() for _ in range(hidden_layer_size)] for _ in range(sensor_num)])
        
        # randomly initilize the output layer weights
        self._output_layer_weights = np.array([[random() for _ in range(motor_num)] for _ in range(hidden_layer_size)])
    ...
    def crossover(a: "RobotGenome", b: "RobotGenome") -> Tuple["RobotGenome", "RobotGenome"]:
        ...
        motor_num = a._output_layer_weights.shape[0] * a._output_layer_weights.shape[1] / hidden_layer_size
        ...
        a_crossover = a._input_layer_weights[0:p_1, 0:p_2] + b._input_layer_weights[p_1:, p_2:]
        new_a._input_layer_weights = a_crossover

        b_crossover = b._input_layer_weights[0:p_1, 0:p_2] + a._input_layer_weights[p_1:, p_2:]
        new_b._input_layer_weights = b_crossover

        # perform crossover for the second layer
        a_crossover = a._output_layer_weights[0:p_2, 0:p_3] + b._output_layer_weights[p_2:, p_3:]
        new_a._output_layer_weights = a_crossover

        b_crossover = b._output_layer_weights[0:p_2, 0:p_3] + a._output_layer_weights[p_2:, p_3:]
