"""Module provides the car sprite based on data in the centimeter dimension."""
import math
...
class CarBase(Sprite):
    """The car base sprite shows the car's essentials and can be modified using the setter and 
    classe's constant variables. 

    Args:
        Sprite (Sprite): The base sprite object.

    Returns:
        CarBase: The car can be displayed by attatching it to a window or calling the draw() method.
    """
    ...
    def __init__(self, surface: py.Surface, position: Tuple[int, int], size: Tuple[int, int],
        ...
        self._background = Rectangle(
            self._surface, position[0], position[1], size[0], size[1], (40, 45, 45))
    ...
    def _get_car_size(self) -> Tuple[float, float]:
        ...
        car_height = self._data[CarBase.WHEELBASE][0] + self._data[CarBase.WHEEL_DIAMETER][0] + \
            self._data[CarBase.CHASSIS_FRONT][0] + \
            self._data[CarBase.CHASSIS_REAR][0]
    ...
    def _center_sprite(self) -> None:
        ...
        self.__y = self._y + (self._h - car_height) / 2 + \
            self._data[CarBase.CHASSIS_FRONT][1]
    ...
    def _update_sizes(self) -> None:
        ...
        self._data[CarBase.STEERING_ANGLE][1] = math.radians(
            180 - self._data[CarBase.STEERING_ANGLE][0])
        self._angle_coordinates = self._calculate_steering()

    def set_value(self, name: str, value: float) -> None:
        """This method sets an internal value of the car to modify its size.

        Args:
            name (str): The name should be one of CarBase.CONSTANT (e.g. CarBase.WHEELBASE).
            value (float): The value related to the given constant name.
        """
    ...
    def _get_axis_position(self) -> Tuple:
        ...
        w = x + self._data[CarBase.TRACK_SPACING][1] - \
            self._data[CarBase.WHEEL_WIDTH][1]
    ...
    def _get_wheels(self) -> Tuple:
        ...
        x1, x2 = self.__x, self.__x + \
            ...
            self._data[CarBase.WHEEL_WIDTH][1]
        y2 = self.__y + self._data[CarBase.WHEELBASE][1]
    ...
    def _calculate_steering(self) -> Tuple:
        ...
        offset = np.array(
            (self._data[CarBase.TRACK_SPACING][1] - self._data[CarBase.WHEEL_WIDTH][1], 0))
    ...
    def draw(self) -> None:
        ...
        if self._angle_coordinates:
            ...
            py.draw.line(self._surface, (255, 255, 255), s1, e1,
                         width=int(self._data[CarBase.WHEEL_WIDTH][1]))
            py.draw.line(self._surface, (255, 255, 255), s2, e2,
                         width=int(self._data[CarBase.WHEEL_WIDTH][1]))

        # draw axis
        front_axis, rear_axis, center = self._get_axis_position()
        py.draw.line(self._surface, (255, 255, 255),
                     front_axis[:2], front_axis[2:], width=5)
        py.draw.line(self._surface, (255, 255, 255),
                     rear_axis[:2], rear_axis[2:], width=5)
        py.draw.line(self._surface, (255, 255, 255),
                     center[:2], center[2:], width=5)
